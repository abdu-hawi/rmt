...
<body>
    <nav class="fixed top-0 left-0 right-0 z-50 glass border-b border-slate-800/50" style="font-family: 'Cairo', 'Tajawal', sans-serif;">
                ...
                <div class="flex items-center gap-8">
                    ...
                    <div class="hidden md:flex items-center gap-1">
                        ...
                        <a href="{{ route('cart.index') }}" class="relative px-3 py-2 text-sm text-muted-400 hover:text-white hover:bg-white/5 rounded-lg transition-all duration-200">
                            ...
                            @if($cartCount > 0)
                                <span class="absolute -top-0.5 -left-0.5 w-5 h-5 flex items-center justify-center bg-accent text-white text-[10px] font-bold rounded-full">{{ $cartCount }}</span>
                            @endif
                        </a>
                    </div>
                </div>
    ...
    <footer class="border-t border-slate-800/50 bg-deep-950">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-8 text-center">
            <p class="text-sm text-muted-400">&copy; {{ date('Y') }} <span class="text-white font-semibold">ريوف</span>. {{ __('All rights reserved') }}.</p>
        </div>
    </footer>
</body>
</html>
